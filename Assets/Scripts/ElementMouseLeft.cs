...
public class ElementMouseLeft : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            Destroy(gameObject);
            Instantiate(combinedElement2, DetectGroundHeight(other.transform.position.x, other.transform.position.z) - new Vector3(0, 1.3f, 0), combinedElement2.transform.rotation); // Spawns the second combined element
        }
        else if (other.tag == "Enemy") // Checks if the element collides with an enemy or the ground
        {
            Destroy(gameObject);
            Instantiate(elementArea, DetectGroundHeight(other.transform.position.x, other.transform.position.z) - new Vector3(0, 1.1f, 0), elementArea.transform.rotation); // Spawns the element area of this element
        }
        else if (other.tag == "Ground")
        {
            Destroy(gameObject);
            Instantiate(elementArea, transform.position - new Vector3(0, 0.23f, 0) /*DetectGroundHeight(transform.position.x, transform.position.z) - new Vector3(0, 0.8f, 0)*/, elementArea.transform.rotation); // Spawns the element area of this element
