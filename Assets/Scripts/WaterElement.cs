...
public class WaterElement : MonoBehaviour
{
    ...
    void Update()
    {
        transform.Translate(new Vector3(0, -0.3f, 1) * speed * Time.deltaTime); // Moves the element forward and to the ground
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.tag == combiningElement2Tag || other.tag == combiningArea2Tag)
        {
            ...
            Instantiate(combinedElement2, DetectGroundHeight(other.transform.position.x, other.transform.position.z) - new Vector3(0, 3.06f, 0), combinedElement2.transform.rotation);
        }
        else if (other.tag == "Enemy") // Checks if the element collides with an enemy or the ground
        {
            ...
            Destroy(gameObject);
            // Spawns the element area at the position I get from DetectGroundHeigh minus the Vector3
